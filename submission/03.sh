# What is the address of the P2SH script wrapping the witness redeem script in this transaction
transaction=""


redeem=$(bitcoin-cli -regtest decoderawtransaction $transaction | jq -r '.vin[0].scriptSig.asm' | awk '{print $NF}')
hash160=$(echo "$redeem" | xxd -r -p | openssl dgst -ripemd160 -binary | xxd -p -c 256)
payload="c4$hash160"
checksum=$(echo -n "$payload" | xxd -r -p | openssl dgst -sha256 -binary | openssl dgst -sha256 -binary | xxd -p -c 256 | head -c 8)
final_hex="$payload$checksum"
echo "$final_hex" | xxd -r -p | base58
